// src/commands/cln/txsend.rs
use std::sync::Arc;

use cln_rpc::model::requests::TxsendRequest;
use cln_rpc::ClnRpc;
use cln_rpc::Request::TxSend;
...
pub async fn run(options: &[CommandDataOption], cln_client: &Arc<Mutex<ClnRpc>>) -> String {
    ...
    let req = TxsendRequest { txid };

    match cln_client.lock().await.call(TxSend(req)).await {
...
pub fn register(command: &mut CreateApplicationCommand) -> &mut CreateApplicationCommand {
    let options = vec![CommandOptionInfo {
        name: "txid",
        description: "The transaction id to send",
    ...
    command
        .name("cln_txsend")
        .description("Sign and send transaction from txprepare");
